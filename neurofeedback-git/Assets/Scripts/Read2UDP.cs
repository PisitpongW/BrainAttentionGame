	...
	private  void ReceiveData ()
	{
		...
		do {
			try {
				...
				print("Pass BitConverter");

				if(timeController.isStart) // start recording labeling
				{
					print("isStart");
				...
				if (timeTempChanged >= timeshift)
				{
					...
					if (timeController.isSaving) // recording data
					{
						print("isSaving");
				...
				if(timeController.isOnSave) // stop recording labeling
				{
